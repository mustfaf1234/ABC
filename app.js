...
window.exportToPDF = async function () {
  ...
  snapshot.forEach((docSnap, index) => {
    const d = docSnap.data();
    const line = `رحلة ${index + 1}: ${d.name || ""} - ${d.fltno || ""} - ${d.date || ""}`;
    pdf.text(line, 10, y);
    y += 10;
    if (y > 270) {
      pdf.addPage();
