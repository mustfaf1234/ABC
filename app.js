...
window.exportToPDF = async function () { const snapshot = await getDocs(collection(db, "flights")); const { jsPDF } = jspdf; const pdf = new jsPDF(); let y = 10; snapshot.forEach((docSnap, index) => { const d = docSnap.data(); const content = ✈️ رحلة ${index + 1}\nالاسم: ${d.name}\nرقم الرحلة: ${d.fltno}\nالتاريخ: ${d.date}\nملاحظات: ${d.notes}; const lines = pdf.splitTextToSize(content, 180); pdf.text(lines, 10, y); y += lines.length * 10; if (y > 270) { pdf.addPage(); y = 10; } }); pdf.save("flights.pdf"); };
