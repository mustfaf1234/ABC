...
function loadFlightApp() {
  ...
  for (let i = 1; i <= 5; i++) {
    formHTML += `
      <fieldset>
        <legend>✈️ الرحلة ${i}</legend>
        <input id="fltno${i}" placeholder="رقم الرحلة"><br>
        <input id="onchocks${i}" placeholder="ON chocks Time"><br>
        <input id="opendoor${i}" placeholder="Open Door Time"><br>
        <input id="cleanstart${i}" placeholder="Start Cleaning Time"><br>
        <input id="cleanend${i}" placeholder="Complete Cleaning Time"><br>
        <input id="ready${i}" placeholder="Ready Boarding Time"><br>
        <input id="boardingstart${i}" placeholder="Start Boarding Time"><br>
        <input id="boardingend${i}" placeholder="Complete Boarding Time"><br>
        <input id="closedoor${i}" placeholder="Close Door Time"><br>
        <input id="offchocks${i}" placeholder="Off chocks Time"><br>
        <input id="name${i}" placeholder="الاسم"><br>
        <input id="notes${i}" placeholder="ملاحظات"><br>
        <input id="date${i}" type="date"><br>
      </fieldset>
    `;
  }
  ...
  document.getElementById("app").innerHTML = `
    ...
    <hr>
    <input id="filterName" placeholder="بحث بالاسم" oninput="loadFlights()">
    <input id="filterDate" type="date" oninput="loadFlights()">
...
window.saveFlights = async function () {
  ...
  for (let i = 1; i <= 5; i++) {
    const data = {
      fltno: document.getElementById(`fltno${i}`).value.trim(),
      onchocks: document.getElementById(`onchocks${i}`).value.trim(),
      opendoor: document.getElementById(`opendoor${i}`).value.trim(),
      cleanstart: document.getElementById(`cleanstart${i}`).value.trim(),
      cleanend: document.getElementById(`cleanend${i}`).value.trim(),
      ready: document.getElementById(`ready${i}`).value.trim(),
      boardingstart: document.getElementById(`boardingstart${i}`).value.trim(),
      boardingend: document.getElementById(`boardingend${i}`).value.trim(),
      closedoor: document.getElementById(`closedoor${i}`).value.trim(),
      offchocks: document.getElementById(`offchocks${i}`).value.trim(),
      name: document.getElementById(`name${i}`).value.trim(),
      notes: document.getElementById(`notes${i}`).value.trim(),
      date: document.getElementById(`date${i}`).value.trim(),
    };
...
window.loadFlights = async function () {
  ...
  if (name || date) {
    ...
    q = query(collection(db, "flights"), ...conditions);
  }

  const snapshot = await getDocs(q);
  const { jsPDF } = jspdf;
  const monthlyCount = {};
  let html = `<table border="1"><tr><th>تصدير</th><th>حذف</th><th>الاسم</th><th>الرحلة</th><th>التاريخ</th><th>ملاحظات</th></tr>`;

  snapshot.forEach(docSnap => {
    const d = docSnap.data();
    const id = docSnap.id;
    const key = `${d.name || "غير معروف"} - ${(d.date || "").slice(0, 7)}`;
  ...
  if (counterDiv) {
    counterDiv.innerHTML = `<h3>📊 العداد الشهري</h3><ul>` +
      Object.entries(monthlyCount).map(([k, v]) => `<li>${k} ➤ ${v}</li>`).join("") + "</ul>";
  }
};
